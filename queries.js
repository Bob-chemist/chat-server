...
const createMessage = ({ author, id, message, receiver }) => {
    return db.none(
        'INSERT INTO notification (author, id, message, receiver) ' +
              ' VALUES ($1, $2, $3, $4)',
...
const getMessages = (userId, companionId, offset) => {
    console.log(userId, companionId, offset);
    return db.any('' +
        'select * from notification ' +
        ' where ' +
        (companionId.indexOf('_chat') === -1
...
const checkPassword = (code, password) => {
    return (db.any('' +
        'select count(*) from users ' +
        'where userid like $1 and password like $2',
    [ code, password ]
